...
load server_and_tG_0dot01; dist_ktG0dot01=dist;  ss_ktG0dot01=ss;

load server_and_tG_1;  dist_ktG1=dist;  ss_ktG1=ss;

load server_and_tG_10; dist_ktG10=dist; ss_ktG10=ss;




%% plot DoRA trend vs mG level (the middle panel in Figure 3D)
...
for i=0:9
    ...
    if ~isempty(ind)
        val=[val,0.1*i+0.05];
        H_mg=[H_mg,[length(find(trend_mg(ind)==1));length(find(trend_mg(ind)==2));length(find(trend_mg(ind)==3));length(find(trend_mg(ind)==4));length(find(trend_mg(ind)==0))]...
            ./length(ind)];
        H_tg=[H_tg,[length(find(trend_tg(ind)==1));length(find(trend_tg(ind)==2));length(find(trend_tg(ind)==3));length(find(trend_tg(ind)==4));length(find(trend_tg(ind)==0))]...
            ./length(ind)];
    end
end
...
%% plot DoRA metric vs mG level (the bottom panel in Figure 3D)
var_name_d={'dist_ktG0dot01','dist_ktG1','dist_ktG10'};
var_name_s={'ss_ktG0dot01','ss_ktG1','ss_ktG10'};
...
%%  plot mG DoRA metric vs tG DoRA metric (right panel in Figure 5D)
set(0,'DefaultLineLineWidth',1);
set(0,'DefaultAxesFontSize',20,'DefaultAxesFontWeight','bold');
set(0,'DefaultTextFontSize',20,'DefaultTextFontWeight','bold');
...
colormap(col);colorbar;
 ...
 %%  compare AND and OR logic (Figure 4C)
load or_logic/server_or_tG_0dot01
dist_ktG0dot1_and=dist;  ss_ktG0dot1_and=ss;
load or_logic/server_or_tG_1
dist_ktG1_and=dist; ss_ktG1_and=ss;
load or_logic/server_or_tG_10
dist_ktG10_and=dist;   ss_ktG10_and=ss;

load server_and_tG_0dot01
dist_ktG0dot1_or=dist;  ss_ktG0dot1_or=ss;
load server_and_tG_1
dist_ktG1_or=dist; ss_ktG1_or=ss;
load server_and_tG_10
dist_ktG10_or=dist;   ss_ktG10_or=ss;
...
var_name={'ktG0dot1','ktG1','ktG10'};
figure;hold on;set(gcf,'unit','centimeters','position',[0,1,13,8]);  

for var_ind=2
    for cc=1:2
        subplot(1,2,cc);hold on;   s1=0;n1=0;  s2=0;n2=0;
        for i=size(dist,1):-1:1
            for j=size(dist,2):-1:1

                eval(['s1_temp=ss_',var_name{var_ind},'_and{i,j}(ind,2);']);
                eval(['s2_temp=ss_',var_name{var_ind},'_or{i,j}(ind,2);']);

                eval(['d1_temp=dist_',var_name{var_ind},'_and{i,j}(ind,cc);']);
                eval(['d2_temp=dist_',var_name{var_ind},'_or{i,j}(ind,cc);']);
...
subplot(1,2,1);axis([0 0.45 0 0.45]);plot([ 0 0.45 ],[0 0.45])
subplot(1,2,2);axis([0 0.45 0 0.45]);plot([ 0 0.45 ],[0 0.45])
